...
Program* LoadShaders(const ShaderSource& vert_shader_obj, const ShaderSource& frag_shader_obj)
{
	std::vector<Shader> shaders;
	shaders.push_back(Shader(vert_shader_obj, GL_VERTEX_SHADER));
	shaders.push_back(Shader(frag_shader_obj, GL_FRAGMENT_SHADER));
...
void Renderer::initGPUDataBuffers()
{
	...
	glBufferData(GL_TEXTURE_BUFFER, sizeof(glm::vec4) * scene->verticesUV_x.size(), &scene->verticesUV_x[0], GL_STATIC_DRAW);
	...
	glBufferData(GL_TEXTURE_BUFFER, sizeof(glm::vec4) * scene->normalsUV_y.size(), &scene->normalsUV_y[0], GL_STATIC_DRAW);
	...
	if (!scene->textures.empty())
	{
		...
		glTexImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_RGBA8, scene->renderOptions.tex_array_width, scene->renderOptions.tex_array_height, scene->textures.size(), 0, GL_RGBA, GL_UNSIGNED_BYTE, &scene->textureMapsArray[0]);
...
void Renderer::initFBOs()
{
	...
	renderSize = scene->renderOptions.render_resolution;
	windowSize = scene->renderOptions.window_resolution;

	tileWidth = scene->renderOptions.tile_width;
	tileHeight = scene->renderOptions.tile_height;
	...
	// For Denoiser
	denoiserInputFramePtr = new glm::vec3[renderSize.x * renderSize.y];
	frameOutputPtr = new glm::vec3[renderSize.x * renderSize.y];
...
{
	ShaderSource vertexShaderSrcObj = Shader::load(shadersDirectory + "common/vertex.glsl");
	ShaderSource pathTraceShaderSrcObj = Shader::load(shadersDirectory + "tile.glsl");
	ShaderSource pathTraceShaderLowResSrcObj = Shader::load(shadersDirectory + "preview.glsl");
	ShaderSource outputShaderSrcObj = Shader::load(shadersDirectory + "output.glsl");
	ShaderSource tonemapShaderSrcObj = Shader::load(shadersDirectory + "tonemap.glsl");

	// Add preprocessor defines for conditional compilation
	std::string pathtraceDefines = "";
	std::string tonemapDefines = "";

	if (scene->renderOptions.enable_env_map && scene->envMap != nullptr)
		pathtraceDefines += "#define OPT_ENVMAP\n";

	if (!scene->lights.empty())
		pathtraceDefines += "#define OPT_LIGHTS\n";

	if (scene->renderOptions.enable_RR)
	{
		pathtraceDefines += "#define OPT_RR\n";
		pathtraceDefines += "#define OPT_RR_DEPTH " + std::to_string(scene->renderOptions.RR_depth) + "\n";
	}

	if (scene->renderOptions.enable_uniform_light)
		pathtraceDefines += "#define OPT_UNIFORM_LIGHT\n";

	if (scene->renderOptions.opengl_normal_map)
		pathtraceDefines += "#define OPT_OPENGL_NORMALMAP\n";

	if (scene->renderOptions.hide_emitters)
		pathtraceDefines += "#define OPT_HIDE_EMITTERS\n";

	if (scene->renderOptions.enable_background)
	{
		pathtraceDefines += "#define OPT_BACKGROUND\n";
		tonemapDefines += "#define OPT_BACKGROUND\n";
	}

	if (scene->renderOptions.transparent_background)
	{
	...
	{
		if ((int)scene->materials[i].alpha_mode != AlphaMode::Opaque)
		{
	...
	if (scene->renderOptions.enable_roughness_mollification)
		pathtraceDefines += "#define OPT_ROUGHNESS_MOLLIFICATION\n";

	for (int i = 0; i < scene->materials.size(); i++)
	{
		if ((int)scene->materials[i].medium_type != MediumType::None)
		{
	...
	if (scene->renderOptions.enable_volume_MIS)
...
void Renderer::render()
{
	...
	if (!scene->dirty && scene->renderOptions.max_spp != -1 && sampleCounter >= scene->renderOptions.max_spp)
		return;
	...
	if (scene->dirty)
	{
		...
		scene->instances_modified = false;
		scene->dirty = false;
		scene->env_map_modified = false;
	}
	else
	{
...
void Renderer::present()
{
	...
	{
		if (scene->renderOptions.enable_denoiser && denoised)
			glBindTexture(GL_TEXTURE_2D, denoisedTexture);
		else
...
{
	int maxSpp = scene->renderOptions.max_spp;
...
void Renderer::update(float secondsElapsed)
{
	...
	if (scene->renderOptions.enable_denoiser && sampleCounter > 1)
	{
		if (!denoised || (frameCounter % (scene->renderOptions.denoiser_frame_cnt * (numTiles.x * numTiles.y)) == 0))
		{
	...
	glUniform1f(glGetUniformLocation(shaderObject, "camera.focalDist"), scene->camera->focal_dist);
	glUniform1f(glGetUniformLocation(shaderObject, "camera.aperture"), scene->camera->aperture);
	glUniform1i(glGetUniformLocation(shaderObject, "enableEnvMap"), scene->envMap == nullptr ? false : scene->renderOptions.enable_env_map);
	glUniform1f(glGetUniformLocation(shaderObject, "envMapIntensity"), scene->renderOptions.env_map_intensity);
	glUniform1f(glGetUniformLocation(shaderObject, "envMapRot"), scene->renderOptions.env_map_rot / 360.0f);
	glUniform1i(glGetUniformLocation(shaderObject, "maxDepth"), scene->renderOptions.max_depth);
	glUniform2f(glGetUniformLocation(shaderObject, "tileOffset"), (float)tile.x * invNumTiles.x, (float)tile.y * invNumTiles.y);
	glUniform3f(glGetUniformLocation(shaderObject, "uniformLightCol"), scene->renderOptions.uniform_light_col.x, scene->renderOptions.uniform_light_col.y, scene->renderOptions.uniform_light_col.z);
	glUniform1f(glGetUniformLocation(shaderObject, "roughnessMollificationAmt"), scene->renderOptions.roughness_mollification_amt);
	...
	glUniform1f(glGetUniformLocation(shaderObject, "envMapIntensity"), scene->renderOptions.env_map_intensity);
	glUniform1f(glGetUniformLocation(shaderObject, "envMapRot"), scene->renderOptions.env_map_rot / 360.0f);
	glUniform1i(glGetUniformLocation(shaderObject, "maxDepth"), scene->dirty ? 2 : scene->renderOptions.max_depth);
	glUniform3f(glGetUniformLocation(shaderObject, "camera.position"), scene->camera->position.x, scene->camera->position.y, scene->camera->position.z);
	glUniform3f(glGetUniformLocation(shaderObject, "uniformLightCol"), scene->renderOptions.uniform_light_col.x, scene->renderOptions.uniform_light_col.y, scene->renderOptions.uniform_light_col.z);
	glUniform1f(glGetUniformLocation(shaderObject, "roughnessMollificationAmt"), scene->renderOptions.roughness_mollification_amt);
	pathTraceShaderLowRes->stopusing();
	...
	glUniform1i(glGetUniformLocation(shaderObject, "enableTonemap"), scene->renderOptions.enable_tonemap);
	glUniform1i(glGetUniformLocation(shaderObject, "enableAces"), scene->renderOptions.enable_aces);
	glUniform1i(glGetUniformLocation(shaderObject, "simpleAcesFit"), scene->renderOptions.simple_aces_fit);
	glUniform3f(glGetUniformLocation(shaderObject, "backgroundCol"), scene->renderOptions.background_col.x, scene->renderOptions.background_col.y, scene->renderOptions.background_col.z);
