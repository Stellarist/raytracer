...
void createModule(PathTracerState& state)
{
	...
	size_t      inputSize = 0;
	const char* input = sutil::getInputData(OPTIX_SAMPLE_NAME, OPTIX_SAMPLE_DIR, "raytracer.cu", inputSize);
...
int main(int argc, char* argv[])
{
	...
	try {
		...
		if (outfile.empty()) {
			GLFWwindow* window = sutil::initUI("raytracer", state.params.width, state.params.height);
